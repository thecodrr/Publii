...
class Renderer {
    ...
    themeIsValid() {
        ...
        let overridedConfigFilePath = UtilsHelper.fileIsOverrided(
            this.inputDir,
            this.themeName,
            configFilePath
        );
        ...
        if (configValidationResult !== true) {
            return (
                "Theme config.json file is invalid: " + configValidationResult
            );
        }
    ...
    emptyOutputDir() {
        if (UtilsHelper.dirExists(this.outputDir)) {
            ...
            for (let file of files) {
                ...
                fs.rmdirSync(path.join(this.outputDir, file), {
                    recursive: true
                });
            }
        } else {
    ...
    setIO() {
        ...
        if (this.previewMode) {
            ...
            if (
                ...
                UtilsHelper.dirExists(this.previewLocation)
            ) {
    ...
    registerThemeHelpers() {
        ...
        let overridedHelpersFilePath = UtilsHelper.fileIsOverrided(
            this.themeDir,
            helpersFilePath
        );
    ...
    loadSiteConfig() {
        ...
        if (
            ...
            this.siteConfig.advanced.openGraphImage.indexOf("https://") ===
                -1 &&
            this.siteConfig.advanced.openGraphImage.indexOf(
                "media/website/"
            ) === -1
        ) {
            let openGraphImage = path.join(
                this.siteConfig.domain,
                "media",
                "website",
                this.siteConfig.advanced.openGraphImage
            ...
            this.siteConfig.advanced.openGraphImage = URLHelper.fixProtocols(
                this.siteConfig.advanced.openGraphImage
            );
        }
    }
    ...
    loadSiteTranslations() {
        ...
        if (fs.existsSync(userTranslationsPath)) {
            this.translations.user = this.parseTranslations(
        ...
        if (fs.existsSync(themeTranslationsPath)) {
            this.translations.theme = this.parseTranslations(
                themeTranslationsPath
            );
        }
    }
     ...
     */
    loadThemeConfig() {
        let themeConfigPath = path.join(
            this.inputDir,
            "config",
            "theme.config.json"
        );
        let tempThemeConfig = Themes.loadThemeConfig(
            themeConfigPath,
            this.themeDir
        );
        ...
        for (let i = 0; i < tempThemeConfig.customConfig.length; i++) {
            ...
            this.themeConfig.customConfig[key] =
                tempThemeConfig.customConfig[i].value;
        }

        for (let i = 0; i < tempThemeConfig.postConfig.length; i++) {
            let key = tempThemeConfig.postConfig[i].name;
            this.themeConfig.postConfig[key] =
                tempThemeConfig.postConfig[i].value;
        }
    }
    ...
    generatePartials() {
        ...
        let userPartials = this.templateHelper.getUserPartials(
            requiredPartials,
            optionalPartials
        );
        let allPartials = requiredPartials
            .concat(optionalPartials)
            .concat(userPartials);

        for (let i = 0; i < allPartials.length; i++) {
            let template = this.templateHelper.loadPartialTemplate(
                allPartials[i] + ".hbs"
            );

            if (!template && optionalPartials.indexOf(allPartials[i]) > -1) {
                let optionalPartialPath = path.join(
                    __dirname,
                    "..",
                    "..",
                    "..",
            ...
            try {
                ...
                this.errorLog.push({
                    message:
                        "An error (1001) occurred during parsing " +
                        allPartials[i] +
                        ".hbs partial file.",
                    desc: e.message
                });
    ...
    generateFrontpage(ampMode = false) {
        ...
        if (totalNumberOfPosts <= postsPerPage || postsPerPage <= 0) {
            ...
            try {
                ...
                this.errorLog.push({
                    message:
                        "An error (1002) occurred during parsing " +
                        inputFile +
                        " file.",
                    desc: e.message
                });
            ...
            for (
                ...
                if (offset > 0) {
                    ...
                    if (
                        globalContext.context.indexOf("index-pagination") === -1
                    ) {
                ...
                if (offset === 0) {
                    ...
                    this.templateHelper.saveOutputHomePaginationFile(
                        currentPage,
                        output
                    );
                }
    ...
    generatePosts(ampMode = false) {
        ...
        if (!ampMode) {
            for (let i = 0; i < postTemplates.length; i++) {
                ...
                if (
                    fileSlug === "" ||
                    !this.themeConfig.postTemplates[fileSlug]
                ) {
                    continue;
                }

                compiledTemplates[fileSlug] = this.compileTemplate(
                    "post-" + fileSlug + ".hbs"
                );
        ...
        for (let i = 0; i < postIDs.length; i++) {
            ...
            if (!ampMode) {
                fileSlug =
                    postTemplates[i] === "" ? "DEFAULT" : postTemplates[i];
            }
            ...
            if (this.siteConfig.advanced.urls.cleanUrls) {
                ...
                globalContext.website.ampUrl =
                    this.siteConfig.domain + "/amp/" + postSlugs[i] + "/";
            }

            globalContext.config.post = this.cachedItems.posts[
                postIDs[i]
            ].postViewConfig;
            ...
            if (ampMode) {
                this.sendProgress(
                    ...
                    "Generating posts (" + (i + 1) + "/" + postIDs.length + ")"
                );
            } else {
                this.sendProgress(
                    Math.ceil(20 + progressIncrease * i),
                    "Generating posts (" + (i + 1) + "/" + postIDs.length + ")"
                );
            }
        }
    ...
    generatePost() {
        ...
        globalContext.website.pageUrl =
            this.siteConfig.domain + "/" + postSlug + ".html";
        globalContext.website.ampUrl =
            this.siteConfig.domain + "/amp/" + postSlug + ".html";
        globalContext.config.post = this.overridePostViewSettings(
     ...
     */
    overridePostViewSettings(
        defaultPostViewConfig,
        postID,
        postPreview = false
    ) {
        ...
        return PostViewSettingsHelper.override(
            postViewSettings,
            defaultPostViewConfig
        );
    }
    ...
    generateTags(ampMode = false) {
        ...
        let tagTemplates = tagsData.map(tagData => {
            ...
            if (tagData.additionalData.template) {
                if (
                    tagData.additionalData.template === "" ||
                    !this.themeConfig.tagTemplates[
                        tagData.additionalData.template
                    ]
                ) {
                    return "DEFAULT";
        ...
        for (let i = 0; i < tagsData.length; i++) {
            ...
            // Detect if we have enough posts to create pagination
            let totalNumberOfPosts = this.cachedItems.tags[tagIDs[i]]
                .postsNumber;
            let postsPerPage = parseInt(
                this.themeConfig.config.tagsPostsPerPage,
                10
            );
            ...
            if (
                ...
                for (
                    ...
                    globalContext.renderer.isFirstPage = currentPage === 1;
                    globalContext.renderer.isLastPage =
                        currentPage === totalPages;

                    if (offset > 0) {
                        if (
                            globalContext.context.indexOf("pagination") === -1
                        ) {
                            globalContext.context.push("pagination");
                        }

                        if (
                            globalContext.context.indexOf("tag-pagination") ===
                            -1
                        ) {
            ...
            if (ampMode) {
                this.sendProgress(
                    Math.ceil(97 + progressIncrease * i),
                ...
                this.sendProgress(
                    Math.ceil(60 + progressIncrease * i),
                    "Generating tag pages (" +
                        (i + 1) +
                        "/" +
                        tagIDs.length +
    ...
    generateAuthors(ampMode = false) {
        ...
        authorsData = authorsData.map(authorData => {
            try {
                ...
                authorData.config = "";
                console.log(
                    "[WARNING] Wrong author #" +
                        authorData.id +
                        " config - invalid JSON value"
                );
            }
        ...
        authorTemplates = authorsData.map(authorData => {
            if (authorData.config && authorData.config.template) {
                if (
                    authorData.config.template === "" ||
                    !this.themeConfig.authorTemplates[
                        authorData.config.template
                    ]
                ) {
                    return "DEFAULT";
        ...
        for (let i = 0; i < authorsData.length; i++) {
            ...
            if (!ampMode) {
                fileSlug =
                    authorTemplates[i] === "" ? "DEFAULT" : authorTemplates[i];
            }

            // Detect if we have enough posts to create pagination
            let totalNumberOfPosts = this.cachedItems.authors[authorsIDs[i]]
                .postsNumber;
            let postsPerPage = parseInt(
                this.themeConfig.config.authorsPostsPerPage,
                10
            );
    ...
    generate404s() {
        ...
        this.templateHelper.saveOutputFile(
            this.siteConfig.advanced.urls.errorPage,
            output
        );
        console.timeEnd("404");
    }
    ...
    generateCSS() {
        ...
        let overridedNewFileName = UtilsHelper.fileIsOverrided(
            this.themeDir,
            newFileName
        );
        ...
        if (this.siteConfig.advanced.cssCompression === 1) {
            ...
            console.log(
                "CSS stats: " +
                    styleCSS.stats.efficiency +
                    " (" +
                    styleCSS.stats.minifiedSize +
                    "/" +
                    styleCSS.stats.originalSize +
                    ")"
            );
    ...
    getThemeVariablesCSS() {
        ...
        let overridedThemeVariablesPath = UtilsHelper.fileIsOverrided(
            this.themeDir,
            themeVariablesPath
        );
        ...
        if (UtilsHelper.fileExists(themeVariablesPath)) {
            try {
                ...
                this.errorLog.push({
                    message:
                        "An error (1003) occurred during preparing CSS theme variables.",
                    desc: e.message
                });
            }
        }
    ...
    getMainCSS() {
        ...
        let overridedCssPath = UtilsHelper.fileIsOverrided(
            this.themeDir,
            cssPath
        );
    ...
    getOverrideCSS() {
        ...
        let overridedOverridePath = UtilsHelper.fileIsOverrided(
            this.themeDir,
            overridePath
        );
        ...
        if (UtilsHelper.fileExists(overridePath)) {
            try {
                ...
                this.errorLog.push({
                    message:
                        "An error (1003) occurred during preparing CSS overrides.",
                    desc: e.message
                });
            }
        }
    ...
    generateFeeds() {
        if (
            !this.siteConfig.advanced.feed.enableRss &&
            !this.siteConfig.advanced.feed.enableJson
        ) {
            return;
        }
    ...
    async generateSitemap() {
        ...
        let sitemapGenerator = new Sitemap(
            this.db,
            this.outputDir,
            this.siteConfig,
            this.themeConfig
    ...
    generateAMP() {
        ...
        this.templateHelper.outputDir = path.join(
            this.templateHelper.outputDir,
            "amp"
        );
    ...
    async copyFiles() {
        ...
        await FilesHelper.copyMediaFiles(
            this.inputDir,
            this.outputDir,
            postIDs
        );
    ...
    createGlobalContext(context) {
        ...
        globalContext.config = URLHelper.prepareSettingsImages(
            this.siteConfig.domain,
            {
