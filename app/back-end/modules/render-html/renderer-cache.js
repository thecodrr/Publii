...
class RendererCache {
    ...
    getTagPostCounts() {
        ...
        for (let tagPostCount of tagsPostCount) {
            this.renderer.cachedItems.tagsPostCounts[tagPostCount.id] =
                tagPostCount.count;
        }
    ...
    getAuthorPostCounts() {
        ...
        for (let countData of authorPostCounts) {
            this.renderer.cachedItems.authorsPostCounts[countData.id] =
                countData.count;
        }
    ...
    getFeaturedPostImages() {
        ...
        featuredImages.map(
            image => new FeaturedImage(image, this.renderer, "featuredImages")
        );
    ...
    getPostTags() {
        ...
        for (let postTag of postTags) {
            if (this.renderer.cachedItems.postTags[postTag.postID]) {
                this.renderer.cachedItems.postTags[postTag.postID].push(
                    postTag.tagID
                );
            } else {
                this.renderer.cachedItems.postTags[postTag.postID] = [
                    postTag.tagID
                ];
            }
        }
    ...
    getPosts() {
        ...
        let postViewConfigObject = JSON.parse(
            JSON.stringify(this.themeConfig.postConfig)
        );

        posts = posts.map(post => {
            let postViewConfig = this.getPostViewSettings(
                postViewConfigObject,
                post.id
            );
