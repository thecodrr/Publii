...
class Themes {
    constructor(appInstance, siteData = false) {
        ...
        if (siteData) {
            this.sitePath = path.join(
                this.sitesDir,
                this.siteName,
                "input",
                "themes"
            );
            this.siteInputPath = path.join(
                this.sitesDir,
                this.siteName,
                "input"
            );
            this.siteConfigPath = path.join(
                this.sitesDir,
                this.siteName,
                "input",
                "config",
    ...
    loadThemes(pathToThemes = false) {
        ...
        for (let i = 0; i < filesAndDirs.length; i++) {
            if (
                filesAndDirs[i][0] === "." ||
                !UtilsHelper.dirExists(path.join(pathToThemes, filesAndDirs[i]))
            ) {
                continue;
            }
    ...
    currentTheme(returnDir = false) {
        ...
        if (!returnDir && siteData.theme) {
            let themeData = fs.readFileSync(
                path.join(this.sitePath, siteData.theme, "config.json")
            );
            themeData = JSON.parse(themeData);
    ...
    changeTheme(newTheme, oldTheme) {
        ...
        if (
            ...
            let newThemeName = newTheme
                ...
                .replace("uninstall-", "")
                .replace("use-", "");
            let backupConfigPath = path.join(
                this.siteInputPath,
                "config",
                "theme." + newThemeName + ".config.json"
            );
        ...
        if (newTheme.indexOf("uninstall-") === 0) {
            ...
            try {
                themeDirStat = fs.statSync(
                    path.join(this.sitePath, themeToRemove)
                );
            } catch (e) {}
        ...
        if (siteData.theme) {
            ...
            if (themeData.postTemplates) {
                ...
                for (let i = 0; i < templateFiles.length; i++) {
                    ...
                    postTemplates.push([
                        fileName,
                        themeData.postTemplates[fileName]
                    ]);
                }
            }
        }
        ...
        if (siteData.theme) {
            ...
            if (themeData.tagTemplates) {
                ...
                for (let i = 0; i < templateFiles.length; i++) {
                    ...
                    tagTemplates.push([
                        fileName,
                        themeData.tagTemplates[fileName]
                    ]);
                }
            }
        }
            ...
            if (themeData.authorTemplates) {
                ...
                for (let i = 0; i < templateFiles.length; i++) {
                    ...
                    authorTemplates.push([
                        fileName,
                        themeData.authorTemplates[fileName]
                    ]);
                }
            }
        }
    ...
    updateThemeConfig(newConfig) {
        ...
        let themeBackupConfigPath = path.join(
            this.sitesDir,
            this.siteName,
            "input",
            "config",
            "theme." + this.currentTheme(true) + ".config.json"
        );
        let themeDefaultConfig = UtilsHelper.loadThemeConfig(
            path.join(this.sitesDir, this.siteName, "input"),
        ...
        for (let i = 0; i < groups.length; i++) {
            ...
            for (let j = 0; j < options.length; j++) {
                if (
                    ...
                    if (
                        themeConfig[groups[i]] &&
                        themeConfig[groups[i]][mediaPathKey]
                    ) {
                        let mediaPath = themeConfig[groups[i]][mediaPathKey];
                        themeConfig[groups[i]][
                            mediaPathKey
                        ] = this.normalizeThemeImagePath(mediaPath);
                    }
                }
            }
        }
    ...
    static loadThemeConfig(themeConfigPath, themeDir) {
        ...
        if (UtilsHelper.fileExists(themeConfigPath)) {
            ...
            try {
                ...
                console.log(
                    "(!) The saved theme config is malformed. Loading default theme config instead."
                );
                return;
            }
            ...
            for (let k = 0; k < optionGroups.length; k++) {
                ...
                if (themeSavedConfig[optionGroups[k]]) {
                    for (let i = 0; i < defaultThemeConfig[group].length; i++) {
                        ...
                        if (
                            typeof themeSavedConfig[group][name] === "undefined"
                        ) {
                            continue;
                        }

                        if (themeSavedConfig[group][name] !== "") {
                            defaultThemeConfig[group][i].value =
                                themeSavedConfig[group][name];
                        }
                    }
                }
            }
            ...
            if (themeSavedConfig.defaultTemplates) {
                defaultThemeConfig.defaultTemplates = JSON.parse(
                    JSON.stringify(themeSavedConfig.defaultTemplates)
                );
            }
        }
    ...
    checkAndCleanImages(configString) {
        ...
        for (let i in images) {
            ...
            if (
                ...
                authorAvatars.indexOf(imagePath.replace(".webp", "")) <= -1 &&
    ...
    removeResponsiveImages(originalPath) {
        ...
        // Load theme config
        let themeConfig = UtilsHelper.loadThemeConfig(
            this.siteInputPath,
            currentTheme
        );
        ...
        if (UtilsHelper.responsiveImagesConfigExists(themeConfig)) {
            dimensions = UtilsHelper.responsiveImagesDimensions(
                themeConfig,
                "optionImages"
            );
            ...
            for (let dimensionName of dimensions) {
                ...
                let responsiveImagePath = path.join(
                    responsiveImagesDir,
                    filename + "-" + dimensionName + extension
                );
