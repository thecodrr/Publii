...
class Image extends Model {
    ...
    generateFileName(fileName, suffix, dirPath, galleryDirPath) {
        ...
        finalFileName =
            slug(finalFileName.name, false, true) +
            fileSuffix +
            finalFileName.ext;
    ...
    save(generateResponsiveImages = true) {
        ...
        if (path.parse(this.path).ext === ".svg") {
            ...
            try {
                ...
                console.log(
                    "back-end/image.js - wrong image path - missing dimensions"
                );
    ...
    createResponsiveImages(originalPath, imageType = "contentImages") {
        ...
        if (
            !siteConfig.advanced.responsiveImages &&
        ...
        if (
            ...
            Utils.responsiveImagesConfigExists(themeConfig, "contentImages")
        ) {
            dimensions = Utils.responsiveImagesDimensions(
                themeConfig,
                "contentImages"
            ...
            imageType === "galleryImages" &&
            Utils.responsiveImagesConfigExists(themeConfig, "galleryImages")
        ) {
            dimensions = Utils.responsiveImagesDimensions(
                themeConfig,
                "galleryImages"
                ...
                if (this.shouldUseJimp()) {
                    result = new Promise((resolve, reject) => {
                        Jimp.read(originalPath, function(err, image) {
                            ...
                            image
                                .scaleToFit(finalWidth, finalHeight)
                                .quality(imagesQuality)
                                .write(destinationPath, function() {
                                    resolve(destinationPath);
                                });
    ...
    saveWebPImage(imagePath, outputPath) {
        return sharp(imagePath).toFile(outputPath + ".webp");
    }
    ...
    getSvgImageDimensions(imagePath) {
        ...
        let svgWidth = svgFileContent.match(
            /\<svg.*width=['"]{1}(.*?)['"]{1}.*\>/im
        );
        let svgHeight = svgFileContent.match(
            /\<svg.*height=['"]{1}(.*?)['"]{1}.*\>/im
        );
        let svgViewBox = svgFileContent.match(
            /\<svg.*viewBox=['"]{1}(.*?)['"]{1}.*\>/im
        );
        ...
        if (
            svgWidth &&
            svgHeight &&
            svgWidth[1].indexOf("%") === 1 &&
            svgHeight[1].indexOf("%") === 1
        ) {
