    ...
    <div 
        ...
        <div class="options-sidebar">
            ...
            <label>
                <span>Instagram:</span>
                <input
                    v-model="authorData.socialLinks.instagram"
                    spellcheck="false"
                    type="text"
                />
            </label>
            ...
            <label>
                <span>Twitter:</span>
                <input
                    v-model="authorData.socialLinks.twitter"
                    spellcheck="false"
                    type="text"
                />
            </label>
...
export default {
    ...
    computed: {
        ...
        metaFieldAttrs: function() {
            ...
            if (!this.metaOptionsActive) {
                text =
                    "To use this option, first enable indexing of author pages in SEO settings.";
            }
        ...
        metaOptionsActive: function() {
            if (
                ...
                this.$store.state.currentSite.config.advanced.metaRobotsAuthors.indexOf(
                    "noindex"
                ) !== -1
            ) {
                return false;
    ...
    mounted() {
        this.$bus.$on("show-author-item-editor", params => {
            ...
            this.authorData.socialLinks = params.socialLinks || {
                facebook: "",
                instagram: "",
                twitter: "",
            };
    ...
    methods: {
        ...
        showMessage(message) {
            ...
            if (message === "author-duplicate-name") {
                ...
                messageConfig.message =
                    "Provided author name is in use. Please try other author name.";
            } else if (message === "author-duplicate-username") {
                this.displayAdvancedOptions = true;
                this.errors.push("slug");
                messageConfig.message =
                    "Provided author name in a similar form (case insensitive) is in use. Please try other author name.";
            } else if (message === "author-empty-name") {
                this.displayAdvancedOptions = false;
                this.errors.push("name");
                messageConfig.message =
                    "Author name cannot be empty. Please try other name.";
            } else if (message === "author-empty-email") {
                this.displayAdvancedOptions = false;
                this.errors.push("email");
                messageConfig.message =
                    "In order to use Gravatar service provide the author e-mail at first.";
                ...
                if (this.authorData.email.trim() !== "") {
                    ...
                    setTimeout(() => {
                        ...
                        this.$bus.$emit("message-display", {
                            message:
                                "Enter the email address you used for registering your account on Gravatar.",
                            type: "warning",
                            lifeTime: 3
                        });
                    }, 100);
        ...
        getGravatar: Utils.debouncedFunction(function() {
            let avatarPath =
                ...
                this.md5(this.authorData.email) +
                "?s=240";
